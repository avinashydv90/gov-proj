...
import { ICasteType } from "../types/ICasteType";
import {
...
const CasteTypeForm: React.FC = () => {
  ...
  const { data, isLoading: isFetching } = useGetCasteTypeByIdQuery(id!, {
    skip: !isEditMode,
  });
  const [addCasteType, { isLoading: isAdding }] = useCreateCasteTypeMutation();
  const [updateCasteType, { isLoading: isUpdating }] = useUpdateCasteTypeMutation();
      ...
      <div className="w-full flex items-center justify-center bg-gray-50 px-4 py-6">
        <div className="w-full max-w-xl bg-white rounded-lg shadow-lg p-8 border border-gray-200">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <label htmlFor="casteName" className="block text-md font-bold text-[#5C4033] mb-1">
                जात प्रकारचे नाव *
              </label>
              <input
                type="text"
